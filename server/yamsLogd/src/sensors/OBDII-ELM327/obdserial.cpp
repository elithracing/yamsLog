...
/// Change some control characters in the data
static void  asciifystring(char *newline,const char *line,int size) {
  int bufpntr = 0;
  int i;
  for(i = 0; i < size ; i++)
  {
    switch(line[i]) {
    case 0x0a:
      sprintf(&(newline[bufpntr]),"\\n");
      bufpntr+=2;
      break;
    case 0x0d:
      sprintf(&(newline[bufpntr]),"\\r");
      bufpntr+=2;
      break;
    default:
      newline[bufpntr++] = line[i];
      break;
    }
  }
  newline[bufpntr] = '\0';
...
static char *see_speed(speed_t speed) {
...
static void appendseriallog(const char *line, int out) {
	if(NULL != seriallog) {
		...
		fprintf(seriallog, "%s(%s): '%s'\n", timestr, out==SERIAL_OUT?"out":"in", fixedline);
		fflush(seriallog);
		free(fixedline);
	}
}
...
// Send a command and look for specific string in the response
/**
 \param cmd command to send
 \param exp_response string to look for
 \param fd file descriptor
 */
static int semiblindcmd(int fd, const char *cmd, const char* exp_response) {
	char outstr[1024];
	snprintf(outstr, sizeof(outstr), "%s%s\0", cmd, OBDCMD_NEWLINE);
	appendseriallog(outstr, SERIAL_OUT);
	write(fd,outstr, strlen(outstr));
	char retbuf[4096]; // Buffer to store returned stuff
	int bytesRead = 0;
	bytesRead = readserialdata(fd, retbuf, sizeof(retbuf));
	if(-1 !=  bytesRead )
	{
	  if(NULL == strstr(retbuf, exp_response)) {
	    fprintf(stderr, "OBD response invalid. Expected: \"%s\" but got: \"%s\"\n", exp_response, retbuf);
	    return -1;
	  }
...
int openserial(const char *portfilename, long baudrate, long baudrate_target) {
	...
	if(fd == -1) {
		...
		printf( "Flushing serial port input buffer...\n");
		char retbuf[200];
		int readBytes = 0;
              	do {
		  readBytes = read(fd, retbuf, sizeof(retbuf));
		  //printf("Bytes read = %i \n",readBytes);
		...
		//printf("Resetting OBD-Device, expecting response like: \"ELM xxx\"\n");
		//blindcmd(fd,"ATZ",1);
		if(-1 == semiblindcmd(fd,"ATZ","ELM"))
		  return -1;
...
static long attempt_upgradebaudrate(int fd, long rate, long previousrate) {
	...
	appendseriallog(brd_cmd, SERIAL_OUT);
...
//		usleep(timeout * 1000); // Give it a chance to upgrade
		...
		appendseriallog(elm_response2, SERIAL_IN);
		// printf("\n\"%s\"\n", elm_response2);
...
long guessbaudrate(int fd) {
	...
	for(i=0; i<sizeof(guesses)/sizeof(guesses[0]); i++) {
		...
		int jj = 0;
		// Do this twice to eliminate bogus responses
		for(jj = 0; jj < 2 ; jj++)
		{
		  int nbytes = write(fd, testcmd, sizeof(testcmd));
		  appendseriallog(testcmd, SERIAL_OUT);
		  if(-1 == nbytes) {
		  ...
		  appendseriallog(retbuf, SERIAL_IN);
		  if(-1 == nbytes) {
		    perror("Error reading from serial port guessing baudrate");
		    return -1;
		  }
		}
		if(NULL != strstr(retbuf, "ELM")) {
...
int modifybaud(int fd, long baudrate) {
	...
	if(baudrate == -1) 
	{
	  speedreq  = cfgetispeed(&options);
	  printf("No baudrate specified, using old baudrate: %s\n",see_speed(speedreq));
	  return 0;
	}
	...
	fprintf(stderr,"Changing baudrate on hardware device to: %i\n",(int) baudrate);
